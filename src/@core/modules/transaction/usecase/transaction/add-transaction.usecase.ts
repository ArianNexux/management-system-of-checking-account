...
export default class AddTransactionUseCase {
    ...
    async execute(input: AddTransactionInputDTO): Promise<AddTransactionOutputDTO> {
        ...
        const balanceAfter = input.type == "Debit" ? Number(balanceAfterLastTransaction) + Number(input.amount) : Number(balanceAfterLastTransaction) - Number(input.amount)
        ...
        const propsTransaction = {
            ...
            type: input.type,
            amount: input.amount,
            balance_after: isNaN(balanceAfter) ? input.type == "Debit" ? input.amount : input.amount : balanceAfter,
