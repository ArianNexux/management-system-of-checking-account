...
export default class AddTransactionUseCase {
    ...
    async execute(input: AddTransactionInputDTO): Promise<AddTransactionOutputDTO> {
        ...
        const balanceAfter = input.type == "Credit" ? balanceAfterLastTransaction + input.amount : balanceAfterLastTransaction - input.amount
        const propsTransaction = {
            id: new Id(),
            expenditure: dataExpenditure,
            type: input.type,
            amount: input.amount,
            balance_after: balanceAfterLastTransaction ? balanceAfter : input.amount,
